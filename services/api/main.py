...
DEBUG_PREDICTIONS = os.getenv("DEBUG_PREDICTIONS", "false").lower() == "true"
try:
...
def _filter_scratched_runners(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Final safety check: If ALL remaining runners have exact defaults, it indicates
    # PuntingForm returned scratched-only data. Return empty to avoid recommending scratched horses.
    if not df.empty and all(col in df.columns for col in ["betfair_horse_rating", "win_rate"]):
...
def _score(df_raw: pd.DataFrame, booster: Booster) -> pd.DataFrame:
    ...
    # Check PuntingForm feature availability (lightweight check on first row only)
    pf_features = ['pf_score', 'neural_rating', 'pf_ai_rank', 'pf_ai_score', 'pf_ai_price']
    # Quick check: if any PF feature exists and has non-null values, use PF features
    use_pf_features = any(
        feat in df_feat.columns and df_feat[feat].notna().any()
        for feat in pf_features
    )

    # Select feature set based on PF availability
    feature_cols = [c for c in get_feature_columns(clean_betfair_only=not use_pf_features) if c in df_feat.columns]

    if DEBUG_PREDICTIONS:
        # Only run expensive logging when debug mode is enabled
...
async def run_ace_endpoint(payload: AceRunRequest) -> AceRunResponse:
    ...
    async with _ace_lock:
        ...
        try:
            # Fetch live data for yesterday (should have complete results)
        ...
        if live_df is None or live_df.empty:
            raise HTTPException(
                status_code=404,
                detail=f"No PF live data available for {target_date}. Try running ACE manually with historical schema data."
            )
        ...
        return AceRunResponse(
            status="completed",
            message=f"ACE run finished successfully (using yesterday's complete results)",
