...
## Context: HorseRacingML Project
This is a horse racing prediction system with the following ML infrastructure:

### Current Stack
- **Model**: LightGBM binary classifier (betfair_kash_top5_model_*.txt)
- **Features**: 21 clean Betfair features (market, form cycle, historical priors)
- **Training Data**: 529,686 runners from Betfair historical data
- **API**: FastAPI serving predictions via `/predictions` endpoint
- **Calibration**: Per-race normalization (probabilities sum to 1.0 per race)
- **ACE**: Autonomous betting strategy engine that tests and validates strategies

### Key Files
- `feature_engineering.py`: Feature engineering pipeline
- `train_model_pf.py`: Model training script
- `services/api/main.py`: Production FastAPI serving
- `services/api/pf_schema_loader.py`: Data loading from PuntingForm API
- `artifacts/models/`: Trained model storage
- `services/api/artifacts/models/`: Production model deployment

### Recent Work
- Fixed inflated probability predictions by retraining on clean features
- Implemented per-race calibration for realistic probability distributions
- Updated confidence thresholds across frontend/backend (25%/15%/10%)
- Achieved Test AUC: 0.866, POT: 90.34% on temporal holdout

### Priorities
1. Model accuracy and realistic probability calibration
2. Production reliability and low-latency inference
3. Feature engineering quality and data validation
4. ACE strategy validation and profitability metrics
5. System monitoring and model performance tracking
